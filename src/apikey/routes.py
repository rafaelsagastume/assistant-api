...
from src.apikey.models import ApiKeyResponse, ApiKeyResponseSuccess
...
@router.delete("/delete/{apikey}", response_model=ApiKeyResponseSuccess)
async def delete(apikey: str, authorization: dict = Depends(verify_token)):

    try:
        organization = authorization.organization
        await delete_api_key(organization, apikey)
        return ApiKeyResponseSuccess(message="API key deleted successfully")
    except HTTPException as e:
        raise e
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
